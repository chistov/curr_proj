...
THREE.TypedGeometry.prototype.setArrays = function ( vertices, normals, colors, uvs ) {
...
THREE.TypedGeometry.prototype.merge = ( function () {
	...
	return function ( geometry, matrix, startOffset ) {
		...
		if ( startOffset !== undefined ) this.offset = startOffset;

		var offset2 = this.offset * 2;
		var offset3 = this.offset * 3;
		...
		if ( geometry instanceof THREE.TypedGeometry ) {
			...
			var colors2 = geometry.attributes.color.array;
			...
			for ( var i = 0, l = vertices2.length; i < l; i += 3 ) {
				...
				colors[ i + offset3     ] = colors2[ i     ];
				colors[ i + offset3 + 1 ] = colors2[ i + 1 ];
				colors[ i + offset3 + 2 ] = colors2[ i + 2 ];
			...
			for ( var i = 0, l = indices2.length; i < l; i ++ ) {
				...
				colors[ i3 + offset3 ] = colors2[ index3 ];
				colors[ i3 + offset3 + 1 ] = colors2[ index3 + 1 ];
				colors[ i3 + offset3 + 2 ] = colors2[ index3 + 2 ];
